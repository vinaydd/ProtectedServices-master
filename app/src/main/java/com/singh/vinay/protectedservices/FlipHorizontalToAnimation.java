...
public class FlipHorizontalToAnimation extends Animation {
	...
	@Override
	public void animate() {
		ViewGroup parentView = (ViewGroup) xc.getParent(), rootView = (ViewGroup) xc
				.getRootView();

		float pivotX, pivotY, flipAngle = 270f, viewWidth = xc.getWidth(), viewHeight = xc
				.getHeight();
		final float originalRotationY = xc.getRotationY();
		switch (pivot) {
		case PIVOT_LEFT:
			pivotX = 0f;
			pivotY = viewHeight / 2;
		...
		xc.setPivotX(pivotX);
		xc.setPivotY(pivotY);
		flipToView.setLayoutParams(xc.getLayoutParams());
		flipToView.setLeft(xc.getLeft());
		flipToView.setTop(xc.getTop());
		...
		if (direction == DIRECTION_RIGHT) {
			...
			flipToAnim.playSequentially(ObjectAnimator.ofFloat(xc,
		...
		flipToAnim.addListener(new AnimatorListenerAdapter() {
			...
			public void onAnimationEnd(Animator animation) {
				xc.setVisibility(View.INVISIBLE);
				xc.setRotationY(originalRotationY);
				if (getListener() != null) {
					getListener()
